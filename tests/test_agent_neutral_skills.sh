...
render_project() {
  ...
  copier copy --trust --defaults \
    --data project_name='Guardrails Test' \
    --data project_slug='guardrails-test' \
    --data project_description='Exercises the Guardrails template' \
    --data language='rust' \
...
test_generated_project_uses_agent_neutral_layout() (
  ...
  for skill in gr-architecture gr-implement gr-plan gr-quiz; do
    test -f "$project/.guardrails/skills/$skill/SKILL.md" || \
...
test_skill_customization_survives_copier_update() (
  ...
  git -C "$source" config user.name 'Guardrails Tests'
  git -C "$source" config user.email 'guardrails@example.com'
  git -C "$source" add .
  git -C "$source" commit --quiet -m 'template v1'
  git -C "$source" tag v1.0.0

  render_project "$source" "$project"
  local_file="$project/.guardrails/skills/gr-plan/local.md"
  canonical_file="$project/.guardrails/skills/gr-plan/SKILL.md"
  printf '\nuser-local-marker\n' >> "$local_file"
  git -C "$project" init --quiet
  git -C "$project" config user.name 'Guardrails Tests'
  git -C "$project" config user.email 'guardrails@example.com'
  git -C "$project" add .
  git -C "$project" commit --quiet -m 'generated project with local customization'

  printf '\ntemplate-v2-marker\n' >> \
    "$source/template/.guardrails/skills/gr-plan/SKILL.md.jinja"
  git -C "$source" add template/.guardrails/skills/gr-plan/SKILL.md.jinja
