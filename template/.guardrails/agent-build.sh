...
key_file=.guardrails/podman/agent-build.env
candidate_file=.guardrails/podman/agent-build.candidate.env
pin_file=.guardrails/agent-pin.env
version_pattern='^[0-9]+\.[0-9]+\.[0-9]+$'

die() { printf 'Guardrails: %s\n' "$*" >&2; exit 1; }

prepare() {
    claude_version=latest
    codex_version=latest
    refresh="${GUARDRAILS_ISO_WEEK:-$(date -u +%G-W%V)}"
    ...
    if [ -e "$pin_file" ]; then
        ...
        while IFS= read -r raw || [ -n "$raw" ]; do
            ...
            [ -n "$line" ] || die "$pin_file contains an empty or malformed line"
            case "$line" in
...
case "${1:-prepare}" in
    ...
    promote)
        ...
        [ "${2:-}" ] && [ "${3:-}" ] || die 'promote requires exact Claude and Codex versions'
