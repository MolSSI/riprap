...
if "%~1"=="--reset-agent-state" (
    if "%~3"=="--yes" (
        powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\credential-state.ps1 -Action Reset -Agent "%~2" -Confirmation "--yes"
    ) else (
        powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\credential-state.ps1 -Action Reset -Agent "%~2"
    )
    exit /b !ERRORLEVEL!
)
if "%~1"=="--install-git-hooks" (
    powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\credential-state.ps1 -Action InstallHooks
    exit /b !ERRORLEVEL!
)

for /f "usebackq delims=" %%i in (`powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\credential-state.ps1 -Action Ensure`) do set PROJECT_ID=%%i
...
set /p IMAGE=<.riprap\podman\image_name

REM Validate the complete pin and write transient candidate state before Podman runs.
powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\agent-build.ps1 prepare
if errorlevel 1 exit /b %ERRORLEVEL%

REM Tooling failures are never treated as refresh failures.
call podman build -t riprap-tooling:latest .riprap\podman
if errorlevel 1 (
    powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\agent-build.ps1 discard
...
call podman build -f .riprap\podman\Agent.Containerfile -t riprap-agent:candidate .riprap\podman
...
call podman build -f .riprap\podman\AgentLabels.Containerfile --build-arg CLAUDE_VERSION=!CLAUDE_VERSION! --build-arg CODEX_VERSION=!CODEX_VERSION! --build-arg TOOLING_IMAGE_ID=!TOOLING_ID! -t riprap-agent:latest .riprap\podman
if errorlevel 1 goto agent_refresh_failed
powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\agent-build.ps1 promote !CLAUDE_VERSION! !CODEX_VERSION!
if errorlevel 1 exit /b %ERRORLEVEL%
...
powershell -NoProfile -ExecutionPolicy Bypass -File .riprap\interface.ps1 -Image %IMAGE% -ProjectId %PROJECT_ID%
