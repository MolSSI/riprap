...
reset() {
    agent=${1:-}
    confirmation=${2:-}
    case "$agent" in claude|codex|all) ;; *) die 'usage: gr.sh --reset-agent-state <claude|codex|all> [--yes]' ;; esac
    validate_id
    if [ "$agent" = all ]; then agents='claude codex'; else agents=$agent; fi
    printf 'Guardrails will remove credential state volumes:\n'
...
install_hooks() {
    ...
    if [ -n "$current" ] && [ "$current" != .guardrails/hooks ]; then
        die "core.hooksPath is already '$current'; leave it unchanged and compose that hook with .guardrails/hooks/check-secrets.sh --staged"
    fi
    git config --local core.hooksPath .guardrails/hooks
    printf 'Guardrails Git hooks installed.\n'
}
