...
Riprap keeps its own requirements traceability namespace outside the distributed `template/`
tree. Generated projects own independent requirements identifiers and registries, so Riprap
implementation annotations cannot be mistaken for project-owned requirements.

## Traceability Rules <!-- rq-3105bde4 -->

- Riprap requirements under the top-level `rqm/` directory use stable `rq-XXXXXXXX`
  identifiers.
- Top-level implementation and tests may reference those identifiers.
- No Riprap identifier appears in any file under `template/`, including template
  implementations, scripts, tests, adapters, or generated documentation.
- An identifier is a Riprap identifier when the top-level requirements registry records it.
  Validation resolves the forbidden set from that registry, so template content is judged against
  the identifiers Riprap actually uses rather than against the identifier syntax.
- Template documentation, skill instructions, and test fixtures may contain illustrative
  identifiers that Riprap does not use. These include non-concrete placeholders such as
...
- The Riprap registry does not record source references from `template/`.
...
```gherkin
Feature: Separate Riprap and generated-project traceability

  @rq-f63c0743
  Scenario: Riprap traceability validation rejects a Riprap ID in the template tree
    Given the requirements registry records a Riprap requirement ID
    And that identifier appears in a file under "template/"
    When Riprap traceability validation runs
  ...
  Scenario: A generated project starts with an independent traceability namespace
    Given the Riprap template contains no Riprap requirement identifiers
    ...
    And the Riprap repository registry is not copied into the generated project
```
